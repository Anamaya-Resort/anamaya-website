...
export default async function EditTestimonial({
  ...
  return (
    <div>
      ...
      >
        <Input name="author"      label="Author"         defaultValue={t.author ?? ""} required />
        <Input name="source"      label="Source"         defaultValue={t.source ?? ""} />
        <Input name="source_date" label="Date"           defaultValue={t.source_date ?? ""} />
        <Input name="rating"      label="Rating (1-5)"   type="number" min={1} max={5} defaultValue={t.rating ?? 5} />
        <Input name="headline"    label="Headline"       className="sm:col-span-2" defaultValue={t.headline ?? ""} />
