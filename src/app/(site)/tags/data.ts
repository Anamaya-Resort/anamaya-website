...
// (Mirrors category/[slug]/data.ts — we count only published v2 posts, but
//  we UNION duplicate v1/v2 terms by slug so a tag that only carries its
//  posts on one copy of the term still shows every story.)
...
export type TagsData = {
  tags: TagInfo[];
  postsByTag: Record<string, TagPost[]>;
  totalTaggedPosts: number;
};

/**
 * Load every `post_tag` term that has >=1 published v2 post, plus the posts
 * grouped under each tag (newest first).
 *
 * Query is kept LOCAL to this route (a handful of small queries stitched in
 * JS) rather than editing the shared queries.ts, matching the category route.
 */
export async function getTagsData(): Promise<TagsData> {
  const empty: TagsData = { tags: [], postsByTag: {}, totalTaggedPosts: 0 };
  const sb = supabaseServerOrNull();
  if (!sb) return empty;

  // 1) Every tag term (across BOTH source sites so we can union v1/v2
  //    duplicates by slug). The WP import can leave more than one
  //    taxonomy_terms row per slug; we collect every id per slug so a tag
  //    whose posts hang off only one copy of the term still resolves.
  ...
  //    ids / drafts — only these count toward a tag).
  ...
  // 4) Authors, media, body content — batched, stitched in JS (same as
  //    the category route). Used to resolve the thumbnail mirror image.
  const authorIds = [
  ...
  // Assemble the reusable TagPost per row.
  const postById = new Map<string, TagPost>();
  for (const r of rows) {
    const media =
  ...
  // 5) Group posts under each tag slug (dedup per slug; newest first via the
  //    ordered `rows` query preserving order below).
  ...
  // rows is already newest-first; use it to order each tag's list.
  const orderedRowIds = rows.map((r) => r.id);

  const tags: TagInfo[] = [];
  const postsByTag: Record<string, TagPost[]> = {};
  for (const [slug, agg] of bySlug) {
    ...
    tags.push({ name: agg.name || slug, slug, count: list.length });
    postsByTag[slug] = list;
  }

  const taggedPostIds = new Set<string>();
  for (const set of postIdsBySlug.values()) {
    for (const id of set) taggedPostIds.add(id);
  }

  return { tags, postsByTag, totalTaggedPosts: taggedPostIds.size };
}
